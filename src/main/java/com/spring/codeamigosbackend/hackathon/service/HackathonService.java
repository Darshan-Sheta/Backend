...
public class HackathonService {
    ...
    public void initGemini() {
        ...
        try {
            ...
            try {
                ...
                logger.debug("Failed to load .env file (this is OK if using system env vars): {}", dotenvError.getMessage());
            }
        } catch (Exception e) {
    ...
    public List<ScoredUser> recommendUsersForHackathon(String hackathonId) {
        try {
            ...
            // More lenient filtering - don't require GitHub username (we'll handle that in fallback)
            ...
            logger.info("✅ Eligible users after filtering: {} (out of {} total)", eligibleUsers.size(), allUsers.size());
            ...
            if (eligibleUsers.isEmpty()) {
                ...
                if (!allUsers.isEmpty()) {
                    ...
                    scoredUser.setName(firstUser.getDisplayName() != null ? firstUser.getDisplayName() : firstUser.getUsername());
            ...
            if (scoredUsers != null && !scoredUsers.isEmpty()) {
                scoredUsers.sort((a, b) -> {
                    ...
                    // Secondary: If scores are equal, sort by number of matched skills (more skills = better)
                ...
                logger.info("✅ Found {} recommended users from Gemini for hackathon {} (sorted by score)", scoredUsers.size(), hackathonId);
                return scoredUsers;
            }
            ...
            if (fallbackResults != null && !fallbackResults.isEmpty()) {
                ...
                System.out.println("✅ First user: " + fallbackResults.get(0).getName() + " (Score: " + fallbackResults.get(0).getScore() + ")");
                System.out.println("========================================");
                logger.info("✅ Fallback: Found {} recommended users for hackathon {} (sorted by score)", fallbackResults.size(), hackathonId);
    ...
    private List<ScoredUser> getBasicRecommendations(Hackathon hackathon, List<User> users) {
        try {
            ...
            for (User user : users) {
                try {
                    ...
                    try {
                        ...
                        logger.debug("Could not get framework stats for user {}: {}", user.getUsername(), e.getMessage());
                        hasFrameworkStats = false;
                    }
            ...
            logger.info("📈 Found {} users with matches, {} users without stats", recommendations.size(), usersWithoutStats.size());
            ...
            if (recommendations.isEmpty()) {
                ...
                if (!usersWithoutStats.isEmpty()) {
                    ...
                    System.out.println("✅ Returning backup user: " + bestBackup.getName() + " (Score: " + bestBackup.getScore() + ")");
            ...
            if (!recommendations.isEmpty()) {
                System.out.println("✅ First user: " + recommendations.get(0).getName() + " (Score: " + recommendations.get(0).getScore() + ")");
            }
            System.out.println("========================================");
            logger.info("✅ Basic recommendations: Found {} users for hackathon (sorted by score, highest first)", recommendations.size());
    ...
    private List<ScoredUser> getAtLeastOneUser(List<User> users) {
        ...
        for (User user : users) {
            if (user != null && user.getId() != null && user.getUsername() != null) {
                ...
                System.out.println("========================================");
                logger.info("✅ Returning at least one user (last resort): {} (ID: {})", scoredUser.getName(), scoredUser.getUserId());
                return Collections.singletonList(scoredUser);
            }
        }
        ...
        for (User user : users) {
            try {
                ...
                String userJson = String.format(
                    ...
                    user.getDisplayName() != null ? user.getDisplayName().replace("\"", "\\\"") : user.getUsername(),
    ...
    private String callGeminiAPI(String prompt) {
        ...
        try {
            ...
            org.springframework.http.ResponseEntity<Map<String, Object>> response = 
                    restTemplate.exchange(url, org.springframework.http.HttpMethod.POST, entity, 
    ...
    private List<ScoredUser> parseGeminiResponse(String response, List<User> users) {
        try {
            ...
            com.fasterxml.jackson.core.type.TypeReference<List<ScoredUser>> typeRef = 
                    new com.fasterxml.jackson.core.type.TypeReference<List<ScoredUser>>() {};
    ...
    private MatchingResult evaluateUserWithGemini(User user, Hackathon hackathon, Map<String, Integer> frameworkUsage) {
        ...
        try {
            ...
            logger.debug("Gemini raw response for user {}: {}", user.getUsername(), text.substring(0, Math.min(200, text.length())));
    ...
    private String buildMatchingPrompt(User user, Hackathon hackathon, Map<String, Integer> frameworkUsage) {
        ...
        prompt.append("You are an expert at matching developers with hackathons based on their technical skills and experience.\n\n");
        ...
        prompt.append("- Display Name: ").append(user.getDisplayName() != null ? user.getDisplayName() : user.getUsername()).append("\n");
